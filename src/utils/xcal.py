...
This file contains the d_calibration function from the official X-CAL repository:
https://github.com/rajesh-lab/X-CAL/blob/master/util/d_calibration.py

Original authors:
    Mark Goldstein, Xintian Han, Aahlad Puli, Adler J. Perotte, Rajesh Ranganath
    "X-CAL: Explicit Calibration for Survival Analysis", NeurIPS 2020.
    Paper: https://papers.nips.cc/paper/2020/hash/d4a93297083a23cc099f7bd6a8621131-Abstract.html
...
def d_calibration(points, is_alive, args=None, nbins=20, differentiable=False, gamma=1.0, device='cpu'):
    """
    Compute D-Calibration penalty for survival model calibration.
    
    This function measures whether the CDF values F_theta(T|x) are uniformly distributed
    in [0,1] by binning them and comparing observed vs. expected counts.
    
    Args:
        points (torch.Tensor): [N] CDF values F_theta(u_i|x_i) where u_i is observed time
                              (event time if not censored, censoring time otherwise).
                              Should be in range (0, 1).
        is_alive (torch.Tensor): [N] censoring indicator where:
                                 - 1 = censored (alive at observation time)
                                 - 0 = event observed
                                 NOTE: This is OPPOSITE of standard convention!
        args: Optional arguments (kept for compatibility with original code, can be None)
        nbins (int): Number of bins to discretize [0,1] interval. Default: 20
        differentiable (bool): If True, use soft (sigmoid-based) membership for gradients.
                              If False, use hard membership (for evaluation only).
                              Default: False
        gamma (float): Temperature parameter for soft membership. Higher values give
                      sharper transitions (closer to hard binning).
                      Typically use 1e3-1e4 for training. Default: 1.0
        device (str): Device to place tensors on ('cpu' or 'cuda'). Default: 'cpu'
    
    Returns:
        torch.Tensor: Scalar calibration penalty. Sum of squared deviations between
                     observed and expected bin counts: Σ_j (frac_in_bin_j - width_j)²
                     Lower is better (0 = perfectly calibrated).
    
    Example:
        >>> # Assume we have model predictions
        >>> cdf_at_times = model.predict_cdf(x, times)  # [batch_size]
        >>> is_censored = 1.0 - events.float()  # Convert to X-CAL convention
        >>> 
        >>> xcal_loss = d_calibration(
        ...     points=cdf_at_times,
        ...     is_alive=is_censored,
        ...     nbins=20,
        ...     differentiable=True,
        ...     gamma=1e3,
        ...     device='cuda'
        ... )
        >>> 
        >>> total_loss = nll_loss + 0.5 * xcal_loss
    """
